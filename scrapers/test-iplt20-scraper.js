/**
 * IPL T20 News Scraper - Test Script
 *
 ...
 *
 * Run: node scrapers/test-iplt20-scraper.js
 */
      ...
      const articleDetails = await page.evaluate(() => {
        ...
        if (main) {
          ...
          elements.forEach((el) => {
            ...
            // These are standalone bold cells like "**Sr. No**" or "**Reserve Price**"
...
async function main() {
  ...
  try {
    ...
    const newsList = await scraper.fetchLatestNews();
    ...
    if (newsList.length > 0) {
      ...
      const articleDetails = await scraper.fetchArticleDetails(
        firstArticle.link
      );
      ...
      console.log(`   Word Count: ${articleDetails.wordCount}`);
      console.log(
        `   Paragraphs: ${articleDetails.contentParagraphs?.length || 0}`
      );
      console.log(
        `   Instagram Embeds: ${articleDetails.embeddedInstagram?.length || 0}`
      );
      console.log(`   PDF Links: ${articleDetails.pdfLinks?.length || 0}`);
      console.log(
        `   Scorecard Links: ${articleDetails.scorecardLinks?.length || 0}`
      );

      if (articleDetails.embeddedInstagram?.length > 0) {
        console.log("\n📸 Instagram Embeds:");
        articleDetails.embeddedInstagram.forEach((ig, i) => {
          console.log(`   ${i + 1}. [${ig.type}] ${ig.url}`);
        });
      }

      if (articleDetails.pdfLinks?.length > 0) {
        console.log("\n📄 PDF Documents:");
        articleDetails.pdfLinks.forEach((pdf, i) => {
      ...
      if (articleDetails.content) {
        ...
        console.log("---");
        console.log(articleDetails.content.substring(0, 800) + "...");
        console.log("---\n");
      }
    }
    ...
    const detailedNews = await scraper.fetchLatestNewsWithDetails(3);
    ...
    detailedNews.forEach((article, index) => {
      ...
      if (details) {
        console.log(
          `   Words: ${details.wordCount}, IG Embeds: ${
            details.embeddedInstagram?.length || 0
          }, PDFs: ${details.pdfLinks?.length || 0}`
        );
      } else {
        console.log(`   ⚠️ Failed to fetch: ${article.fetchError}`);
      }
      console.log("");
    });
    ...
    if (detailedNews.length > 0 && detailedNews[0].details) {
      const displayArticle = { ...detailedNews[0] };
      if (
        displayArticle.details?.content &&
        displayArticle.details.content.length > 800
      ) {
        displayArticle.details.content =
          displayArticle.details.content.substring(0, 800) + "... [truncated]";
      }
